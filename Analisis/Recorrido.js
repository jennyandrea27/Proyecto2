	...
	for(var i=0;i<raiz.hijos.length;i++){
		...
		switch (sent.nombre) {
			case Constante.principal:
				cad_3d+='//DECLARACION DE PRINCIPAL\n';
...
function ejecutar_Sent(cuerpo) {
	...
	for(var i = 0; i< cuerpo.hijos.length;i++){
		...
		case Constante._if:
			...
			cad_3d+='p = p + '+amb_padre.variables.length+' ;\n';
			...
			if(cond.tipo===Constante.tbool){
				...
				error={desc:'Error semantico, evaluar condicion de tipo '+valTipo(cond.tipo),fila:1,col:1};
				insertarError(error);
			}
			cad_3d+='p = p - '+amb_padre.variables.length+' ;\n';
			break;
...
function decVar(sent) {
	...
	var res;
	if(sent.hijos[1].nombre === 'create'){
		//es un objeto
		var nombreElem='init$'+nombreElemento(sent.hijos[0].hijos[0]);
		amb_padre=buscarAmbito(TablaSimbolos[TablaSimbolos.length-1],getAmbito());
		cad_3d+='p = p + '+amb_padre.variables.length+' ;\n';
		//llamado a init
		cad_3d+=nombreElem+'() ;\n';
		...
		cad_3d+=temp_pos+' = p + 0 ;\n';
		cad_3d+=temp_val+' = stack [ '+temp_pos+' ] ;\n';
		cad_3d+='p = p - '+amb_padre.variables.length+' 	;\n';
		res={tipo:Constante.tid,temp:temp_val};
	}else{
...
function asigVar(sent) {
	...
	if(sent.hijos[1].nombre === 'create'){
		//es un objeto
		var nombreElem='init$'+nombreElemento(sent.hijos[0]);
		amb_padre=buscarAmbito(TablaSimbolos[TablaSimbolos.length-1],getAmbito());
		cad_3d+='p = p + '+amb_padre.variables.length+' ;\n';
		//llamado a init
		cad_3d+=nombreElem+'() ;\n';
...
function evaluarExp(exp) {
	switch(exp.nombre){
		...
		case Constante._null:
			...
			return temp;
		case 'create':
			//llamado a init obtener valor de retorno
			temp={tipo:Consta.tid,temp:'temp'};
			return temp;
		case 'lidp':
			return accesoId(exp);
		case '+':
				...
				if(primera.tipo===Constante.tid){
					...
					if(elemento!==null){
						...
						if(atr!==null){
							...
							continuar=false;
							error={desc:'Error semantico, elemento '+primera.tipo_ele+' no tiene atributo '+exp.hijos[cont]+'.',fila:1,col:1};
							insertarError(error);
						}
					}else{
						continuar=false;
						error={desc:'Error semantico, elemento '+primera.tipo_ele+' no ha sido declarado.',fila:1,col:1};
						insertarError(error);
					}
				}else{
					continuar=false;
					error={desc:'Error semantico, la variable '+exp.hijos[cont-1]+' no es de tipo elemento.',fila:1,col:1};
					insertarError(error);
				}
			}
			return primera;
